...
def get_personal_activo():
    conn = get_connection()
    rows = conn.execute(
        "SELECT id, nombre FROM Personal WHERE activo = 1 ORDER BY nombre"
    ).fetchall()
    conn.close()
    return rows
...
def obtener_partes():
    ...
    rows = conn.execute("""
        SELECT
            ...
            p.parte_numero,
            p.fecha_parte,
            p.fecha_registro_utc,
            p.canal_nombre,
            p.total_actividades_min,
            p.hubo_averias,
            p.hubo_paradas,
...
def obtener_parte_por_id(parte_id):
    conn = get_connection()
    parte = conn.execute(
        "SELECT * FROM ParteObra WHERE id = ?", (parte_id,)
    ).fetchone()
    conn.close()
    return parte
